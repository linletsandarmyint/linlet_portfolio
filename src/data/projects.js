export const projects = [
  {
    id: 1,
    title: "MBTI-Based Job Matching Platform",

    category: "Full-Stack + AI",

    description:
      "A full-stack career platform that combines MBTI assessment, skill matching, job recommendations, job applications, and AI-powered career assistance.",

    image: "/projects/mbti-platform.jpg",

    technologies: ["React", "Node.js", "Express", "MongoDB", "JWT", "AI"],

    highlights: [
      "MBTI personality assessment",
      "Job matching algorithm",
      "AI-powered job explanations",
      "Job application management",
      "Admin management system",
    ],

    github: "github.com/linletsandarmyint/MBTI_BASED_JOB_MATCHING",
    liveDemo: "https://mbti-based-job-matching.vercel.app",

    featured: true,
  },

  {
    id: 2,
    title: "Course Management System",

    category: "Full-Stack Application",

    description:
      "A web-based management system designed to manage courses, users, departments, positions, and administrative operations.",

    image: "/projects/course-system.jpg",

    technologies: ["React", "Spring Boot", "Java", "MySQL"],

    highlights: [
      "REST API architecture",
      "Authentication and authorization",
      "Course management",
      "Administrative features",
      "Database integration",
    ],

    github: "#",
    liveDemo: "#",

    featured: true,
  },

  {
    id: 3,
    title: "Digital Marketplace",

    category: "E-Commerce",

    description:
      "A digital marketplace application designed to provide product browsing, shopping, cart management, and user-focused e-commerce functionality.",

    image: "/projects/marketplace.jpg",

    technologies: ["React", "Node.js", "Express", "MongoDB"],

    highlights: [
      "Product management",
      "Shopping cart",
      "User authentication",
      "REST API",
      "Responsive interface",
    ],

    github: "#",
    liveDemo: "#",

    featured: true,
  },
];
